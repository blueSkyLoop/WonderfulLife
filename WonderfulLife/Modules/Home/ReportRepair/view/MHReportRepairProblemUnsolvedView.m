//
//  MHReportRepairProblemUnsolvedView.m
//  WonderfulLife
//
...
@interface MHReportRepairProblemUnsolvedView()

@property (nonatomic,assign)CGFloat keyBoardGap;

@end

@implementation MHReportRepairProblemUnsolvedView
...
- (void)awakeFromNib{
    ...
    self.textBigBgView.layer.borderWidth = 1;
    ...
    [RACObserve(self.textView, text)  subscribeNext:^(NSString *text) {
        ...
        if(atext.length <= 50){
            ...
            atext = [atext substringWithRange:NSMakeRange(0, 50)];
            self.textView.text = atext;

        }
        if(atext.length && atext.length <= 50){
...
- (IBAction)buttonClikAction:(UIButton *)sender {
    ...
    if(self.rightButton == sender){
        if(self.solvedBlock){
            self.solvedBlock(self.textView.text);
        }
    }
