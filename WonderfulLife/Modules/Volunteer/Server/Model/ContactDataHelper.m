...
#import "MHVoAttendanceRecordDetailModel.h"//model
...
+ (NSMutableArray *) getFriendListDataBy:(NSMutableArray *)array{
    ...
    NSArray *serializeArray = [(NSArray *)array sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {//排序
        int i;
        NSString *strA = ((MHVoAttendanceRecordDetailCrewModel *)obj1).pinyin;
        NSString *strB = ((MHVoAttendanceRecordDetailCrewModel *)obj2).pinyin;
    ...
    for (MHVoAttendanceRecordDetailCrewModel *user in serializeArray) {
...
+ (NSMutableArray *)getFriendListSectionBy:(NSMutableArray *)array{
    ...
    for (NSArray *item in array) {
        MHVoAttendanceRecordDetailCrewModel *user = [item objectAtIndex:0];
        char c = [user.pinyin characterAtIndex:0];
        if (!isalpha(c)) {
            c = '#';
        }
