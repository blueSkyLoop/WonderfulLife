...
- (void)deleteImage
{
    ...
    UIImage *deleteImage = self.images[self.tag];
    [self.images removeObject:deleteImage];
    self.photosView.images = self.images;
    
    if ([self.photosView.delegate respondsToSelector:@selector(didDeleteImageIndex:Image:)]) {
        [self.photosView.delegate didDeleteImageIndex:self.tag Image:deleteImage];
    }
    
    if (self.images.count == 2) {
        if ([self.photosView.delegate respondsToSelector:@selector(reloadTableViewHeight)]) {
            [self.photosView.delegate reloadTableViewHeight];
        }
    }
}
