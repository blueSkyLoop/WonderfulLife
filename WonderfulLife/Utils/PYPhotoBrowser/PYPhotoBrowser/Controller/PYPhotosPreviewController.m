...
+ (instancetype)previewController
{
    ...
    [backButton addTarget:readerVc action:@selector(backAction) forControlEvents:UIControlEventTouchUpInside];
    readerVc.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:backButton];
...
- (void)deleteImage
{
//    __block NSInteger page = self.collectionView.contentOffset.x / self.collectionView.py_width + 0.5;
//    // 取出可见cell
//    // 判断即将显示哪一张
//    NSIndexPath *currentIndexPath = [NSIndexPath indexPathForItem:page inSection:0];
    NSArray *visitIndexPath = [self.collectionView indexPathsForVisibleItems];
    NSIndexPath *currentIndexPath = [visitIndexPath lastObject];
    if(!currentIndexPath) return;
    __block NSInteger page = currentIndexPath.row;
    // 移除数组中的某个元素
    UIImage *image = self.selectedPhotoView.photosView.images[page];
    [self.selectedPhotoView.photosView.images removeObjectAtIndex:page];
    
    if ([self.selectedPhotoView.photosView.delegate respondsToSelector:@selector(didDeleteImageIndex:Image:)]) {
        [self.selectedPhotoView.photosView.delegate didDeleteImageIndex:page Image:image];
        
    }
    __weak __typeof(self)weakSelf = self;
    [self.collectionView performBatchUpdates:^{
        [weakSelf.collectionView deleteItemsAtIndexPaths:@[currentIndexPath]];
    } completion:^(BOOL finished) {
        if(page == weakSelf.selectedPhotoView.photosView.images.count){
            page --;
        }
        weakSelf.title = [NSString stringWithFormat:@"%zd/%zd", page + 1, weakSelf.selectedPhotoView.photosView.images.count];
    }];
    ...
    if (self.selectedPhotoView.photosView.images.count == 2) {
        if ([self.selectedPhotoView.photosView.delegate respondsToSelector:@selector(reloadTableViewHeight)]) {
            [self.selectedPhotoView.photosView.delegate reloadTableViewHeight];
        }
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (!self.isStatusBarHidden) [self changeNavBarState];
     // 设置标题
    NSArray *visitIndexPath = [self.collectionView indexPathsForVisibleItems];
    NSIndexPath *indextPath = [visitIndexPath lastObject];
    if(indextPath){
        self.title = [NSString stringWithFormat:@"%zd/%zd", indextPath.row + 1, self.selectedPhotoView.photosView.images.count];
    }
}
